...
mycall void add_sag_step(tr_pstep &pstep, t_bits mask) {
...
// var
  ...
  if ((rol!=0) && (rol!=bits)) {
    ...
    add_gs_step(pstep, mask, ~lo_mask);
    add_gs_step(pstep, ~mask, lo_mask);
    }
  }

mycall void add_sag(tr_imp &imp, t_bits mask) {

  add_sag_step(imp.a_step[imp.nr_step], mask);
  finish_step(imp);
  }

mycall void add_inv_sag_step(tr_pstep &pstep, t_bits mask) {
// Add a sheep and goats step
// var
  t_bits lo_mask;
  t_int rol;
  ...
  if ((rol!=0) && (rol!=bits)) {
    lo_mask = (lo_bit << rol)-1;
    add_gs_step(pstep, ~lo_mask, mask);
    add_gs_step(pstep, lo_mask, ~mask);
    }
  }

mycall void add_inv_sag(tr_imp &imp, t_bits mask) {
...
// var
  ...
  do {
    ...
    while (true) {
      ...
      my_cost_rol =
          // -(t_int)((mul & 1)!=0)*options.cost_rotate
          +nr_1bits(mul)*(
            +options.cost_and
            +options.cost_rotate
            +options.cost_or
...
// var
  ...
  init_imp(imp,perm);
  concat_description(imp.description, "Sheep-and-goats method (Knuth)");
...
mycall void route_merge(tr_imp &imp, bool inv) {
// Original algorithm:
...
// var
  ...
  t_int count;
  tr_ll ll[ld_bits+1];
  t_int i,i1,i2,j,j1,j2,k,idx;
  t_bits mask_src;
  t_int max_src,max_tgt;
  tpr_ll p_ll,p_ll_new;
  tpr_l p_l,p_l1,p_l2;
  ...
  for (i = 0; i<=bits-1; ++i) {
    if (perm1[i] < idx) {
      ...
      p_ll->l[p_ll->count-1]=(tpr_l) malloc(sizeof(tr_l));
  ...
  p_ll->l[p_ll->count] = (tpr_l) malloc(sizeof(tr_l));
  ...
  while (p_ll->count>1) {
    ...
    // Source
    i1=0;
    i2=(p_ll->count+1) / 2;  // odd count => use empty list at end
    // Target
    p_ll_new = &ll[k+1];
    p_ll_new->count=i2;
    for (i=0; i<=i2; ++i) {  // yes, one more; <ld_bits
      p_ll_new->l[i] = (tpr_l) malloc(sizeof(tr_l));
      p_ll_new->l[i]->count=0;
      }
    p_ll->mask=0;
    mask_src=1;
    i=0;
    while (i<p_ll_new->count) {
      // system.write(i1,'+',i2,'  ');
      p_l1 = p_ll->l[i1];
      p_l2 = p_ll->l[i2];
      p_ll_new->l[i] = (tpr_l) malloc(sizeof(tr_l));
      ...
      while (true) {
        if ((j1<p_l1->count) && (j2<p_l2->count)) {
          if (p_l1->idx[j1] < p_l2->idx[j2]) {
            ...
            j=j+1;
            j2=j2+1;
            p_ll->mask = p_ll->mask | mask_src;
            }
          }
        else if (j1<p_l1->count) {
          p_l->idx[j]=p_l1->idx[j1];
          j=j+1;
          j1=j1+1;
          }
        else if (j2<p_l2->count) {
          p_l->idx[j]=p_l2->idx[j2];
          j=j+1;
          j2=j2+1;
          p_ll->mask = p_ll->mask | mask_src;
